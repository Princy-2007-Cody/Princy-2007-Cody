## Hi there 👋

<!--**Princy-2007-Cody/Princy-2007-Cody** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile-->

Here are some ideas to get you started:

- 🔭 I’m currently working on leetcode
- 🌱 I’m currently learning WEB DEVELOPMENT / C++
- 👯 I’m looking to collaborate on GITHUB
- 🤔 I’m looking for help with ...
- 💬 Ask me about Anything you like to ask
- 📫 How to reach me: On my Instagram account
- 😄 Pronouns: He/Him
- ⚡ Fun fact: I am a Childish Gen-Z Developer

<!--
- [My profile site](https://<your-username>.github.io/github-profile/)


## 🌐 Socials:
[![Instagram](https://img.shields.io/badge/Instagram-%23E4405F.svg?logo=Instagram&logoColor=white)](https://instagram.com/itzz.prince_2404) 

# 💻 Tech Stack:
![C](https://img.shields.io/badge/c-%2300599C.svg?style=for-the-badge&logo=c&logoColor=white) ![C++](https://img.shields.io/badge/c++-%2300599C.svg?style=for-the-badge&logo=c%2B%2B&logoColor=white) ![HTML5](https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white) ![CSS3](https://img.shields.io/badge/css3-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white) ![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E) ![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54) ![Vercel](https://img.shields.io/badge/vercel-%23000000.svg?style=for-the-badge&logo=vercel&logoColor=white)
<!--# 📊 GitHub Stats:
![](https://github-readme-stats.shion.dev/api?username=Princy-2007-Cody&theme=shadow_green&hide_border=false&include_all_commits=false&count_private=false)<br/>
![](https://streak-stats.demolab.com/?user=Princy-2007-Cody&theme=shadow_green&hide_border=false)<br/>
![](https://github-readme-stats.shion.dev/api/top-langs/?username=Princy-2007-Cody&theme=shadow_green&hide_border=false&include_all_commits=false&count_private=false&layout=compact)
-->

<!--
[![](https://komarev.com/ghpvc/?username=Princy-2007-Cody&icon=0&color=0)](https://visitcount.itsvg.in)-->

<!-- Proudly created with GPRM ( https://gprm.itsvg.in ) -->

<!-- Proudly created with GPRM ( https://gprm.itsvg.in ) -->


<div align="center">

![Header](https://capsule-render.com/api?type=waving&color=0:0F0C29,50:302B63,100:24243E&height=220&section=header&text=PRINCE%20%7C%20SOFTWARE%20ENGINEER&fontSize=40&fontColor=C9A7FF&animation=fadeIn&fontAlignY=38&desc=AI%20%2F%20ML%20Engineer%20%7C%20Full%20Stack%20Developer%20%7C%20Product%20Builder&descAlignY=58&descSize=18)

<a href="https://github.com/Princy-2007-Cody">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=24&duration=3000&pause=1000&color=A78BFA&center=true&vCenter=true&width=650&lines=Building+AI-powered+products+end-to-end;Precision+Agriculture+%7C+Machine+Learning;Full+Stack+%2B+Systems+Engineering;Turning+Hackathon+Ideas+Into+Shipped+Products" alt="Typing SVG" />
</a>

<br/>

![Student Developer](https://img.shields.io/badge/Student%20Developer-4C1D95?style=flat-square&labelColor=1E1B4B&color=6D28D9)
![ISRO BAH 2026](https://img.shields.io/badge/ISRO%20Bharatiya%20Antariksh%20Hackathon%202026-4C1D95?style=flat-square&labelColor=1E1B4B&color=7C3AED)
![Location](https://img.shields.io/badge/Bhubaneswar%2C%20Odisha%2C%20India-4C1D95?style=flat-square&labelColor=1E1B4B&color=8B5CF6&logo=googlemaps&logoColor=white)

<br/>

[![Portfolio](https://img.shields.io/badge/Portfolio-7C3AED?style=for-the-badge&logo=vercel&logoColor=white)](https://your-portfolio-link.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-6D28D9?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/YOUR_LINKEDIN)
[![Email](https://img.shields.io/badge/Email-5B21B6?style=for-the-badge&logo=gmail&logoColor=white)](mailto:your.email@example.com)
[![GitHub](https://img.shields.io/badge/GitHub-4C1D95?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Princy-2007-Cody)

<br/>

![Profile Views](https://komarev.com/ghpvc/?username=Princy-2007-Cody&style=flat-square&color=7c3aed&label=PROFILE+VIEWS)
![Followers](https://img.shields.io/github/followers/Princy-2007-Cody?style=flat-square&color=8b5cf6&label=FOLLOWERS&labelColor=1E1B4B)
![Stars](https://img.shields.io/github/stars/Princy-2007-Cody?style=flat-square&color=a78bfa&label=STARS&labelColor=1E1B4B)

</div>

---

### 🧠 About Me

```
I engineer end-to-end AI systems and full-stack products — from raw satellite
data pipelines to polished, deployable web applications.
```

I'm a software engineer focused on **applied AI/ML**, **full-stack product engineering**, and **systems that ship**. My work spans multi-source geospatial data fusion for precision agriculture, browser-based productivity tools built around real-world workflow metaphors, and low-level systems programming in C++. I care less about demos and more about **reproducible pipelines, clean architecture, and products a real user could actually rely on.**

I approach engineering with a **product mindset** — every model, dashboard, or interface I build is designed to be understood, trusted, and used, not just technically correct.

<div align="center">

**🎯 Open To:** AI/ML Engineering · Full Stack Development · Freelance Product Builds · Research Collaborations · Hackathons

</div>

---

### 🛠️ Tech Stack

**Languages**

![Python](https://img.shields.io/badge/-Python-1E1B4B?style=flat-square&logo=python&logoColor=8B5CF6)
![C++](https://img.shields.io/badge/-C%2B%2B-1E1B4B?style=flat-square&logo=cplusplus&logoColor=8B5CF6)
![JavaScript](https://img.shields.io/badge/-JavaScript-1E1B4B?style=flat-square&logo=javascript&logoColor=8B5CF6)
![TypeScript](https://img.shields.io/badge/-TypeScript-1E1B4B?style=flat-square&logo=typescript&logoColor=8B5CF6)
![SQL](https://img.shields.io/badge/-SQL-1E1B4B?style=flat-square&logo=postgresql&logoColor=8B5CF6)

**Frontend**

![React](https://img.shields.io/badge/-React-1E1B4B?style=flat-square&logo=react&logoColor=8B5CF6)
![Three.js](https://img.shields.io/badge/-Three.js-1E1B4B?style=flat-square&logo=threedotjs&logoColor=8B5CF6)
![HTML5](https://img.shields.io/badge/-HTML5-1E1B4B?style=flat-square&logo=html5&logoColor=8B5CF6)
![CSS3](https://img.shields.io/badge/-CSS3-1E1B4B?style=flat-square&logo=css3&logoColor=8B5CF6)
![TailwindCSS](https://img.shields.io/badge/-TailwindCSS-1E1B4B?style=flat-square&logo=tailwindcss&logoColor=8B5CF6)

**Backend & Databases**

![Node.js](https://img.shields.io/badge/-Node.js-1E1B4B?style=flat-square&logo=nodedotjs&logoColor=8B5CF6)
![Flask](https://img.shields.io/badge/-Flask-1E1B4B?style=flat-square&logo=flask&logoColor=8B5CF6)
![MongoDB](https://img.shields.io/badge/-MongoDB-1E1B4B?style=flat-square&logo=mongodb&logoColor=8B5CF6)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-1E1B4B?style=flat-square&logo=postgresql&logoColor=8B5CF6)

**Cloud, DevOps & Tooling**

![Git](https://img.shields.io/badge/-Git-1E1B4B?style=flat-square&logo=git&logoColor=8B5CF6)
![GitHub Actions](https://img.shields.io/badge/-GitHub%20Actions-1E1B4B?style=flat-square&logo=githubactions&logoColor=8B5CF6)
![Docker](https://img.shields.io/badge/-Docker-1E1B4B?style=flat-square&logo=docker&logoColor=8B5CF6)
![Vercel](https://img.shields.io/badge/-Vercel-1E1B4B?style=flat-square&logo=vercel&logoColor=8B5CF6)
![Linux](https://img.shields.io/badge/-Linux-1E1B4B?style=flat-square&logo=linux&logoColor=8B5CF6)

<div align="center">
<img src="https://skillicons.dev/icons?i=python,cpp,js,ts,react,threejs,html,css,tailwind,nodejs,flask,mongodb,postgresql,git,docker,vercel,linux&theme=dark" />
</div>

---

### 🤖 AI / ML Expertise

| Domain | Proficiency | Details |
|---|:---:|---|
| **Machine Learning Pipelines** | ⭐⭐⭐⭐⭐ | End-to-end pipeline design — ingestion, feature engineering, training, evaluation, reproducibility (fixed-seed experiments) |
| **Ensemble Modeling** | ⭐⭐⭐⭐⭐ | Random Forest & LightGBM comparative modeling, feature importance analysis, hyperparameter tuning |
| **Geospatial Data Fusion** | ⭐⭐⭐⭐ | Multi-source satellite data fusion for agricultural phenology and stress detection |
| **Feature Engineering** | ⭐⭐⭐⭐ | Domain-driven feature design (e.g., temporal amplitude/day-of-year signals) for crop-stage prediction |
| **Data Visualization for ML** | ⭐⭐⭐⭐ | Building live dashboards that consume model output (CSV/JSON) for decision support |
| **Applied Research Translation** | ⭐⭐⭐⭐ | Converting research-grade pipelines into hackathon-ready, presentation-ready deliverables |

---

### 🚀 Featured Projects

<details>
<summary><b>🌾 Crop-Sense AI — Precision Agriculture Intelligence Platform</b></summary>
<br/>

Multi-source satellite data fusion and machine learning pipeline built for the **ISRO Bharatiya Antariksh Hackathon 2026**, designed to predict crop phenology stages and irrigation needs from remote sensing signals.

| Aspect | Detail |
|---|---|
| **Stack** | Python, Random Forest, LightGBM, Pandas, NumPy, JavaScript dashboard |
| **Scale** | 160 fields across 4 crop types, multi-source satellite ingestion |
| **Performance** | Random Forest ~65% accuracy, LightGBM ~57% accuracy, top feature: `amplitude_doy` |
| **Security** | Deterministic, reproducible experiments (fixed seed = 42) |
| **Impact** | Instrument-panel dashboard with draggable phenology timeline, moisture stress gauge, and irrigation decision engine |
| **Repository** | [github.com/Princy-2007-Cody/crop-sense-ai](https://github.com/Princy-2007-Cody/crop-sense-ai) |

Built as a full ML pipeline paired with a browser-based, cockpit-style dashboard that ingests live model output via CSV/JSON, enabling real-time visualization of phenology stage progression and irrigation recommendations for precision agriculture use cases.

</details>

<details>
<summary><b>🩺 TRIAGE — AI-Powered Productivity Web App</b></summary>
<br/>

A browser-based productivity application built entirely with HTML, CSS, and JavaScript, organized around a **medical triage metaphor** — prioritizing tasks the way an ER prioritizes patients.

| Aspect | Detail |
|---|---|
| **Stack** | HTML5, CSS3, Vanilla JavaScript |
| **Scale** | Fully client-side, zero-dependency productivity engine |
| **Performance** | Lightweight, instant-load single-page application |
| **Security** | No external data transmission — fully local-first design |
| **Impact** | Reframes task prioritization using an urgency/severity triage model for faster decision-making |
| **Repository** | [github.com/Princy-2007-Cody/triage](https://github.com/Princy-2007-Cody/triage) |

Shipped as a complete, standalone productivity tool with accompanying professional documentation, demonstrating end-to-end product thinking from concept to polished deliverable.

</details>

<details>
<summary><b>🌲 Dusk Hollow — Browser-Based 3D Open-World Exploration Game</b></summary>
<br/>

A real-time 3D exploration game built with **React** and **Three.js**, featuring procedural terrain generation and a living day/night cycle.

| Aspect | Detail |
|---|---|
| **Stack** | React, Three.js, JavaScript, WebGL |
| **Scale** | Procedurally generated open-world terrain with dynamic lighting |
| **Performance** | Optimized geometry usage after resolving `THREE.CapsuleGeometry` compatibility issues |
| **Security** | Client-side rendering, no backend attack surface |
| **Impact** | NPC deer with wandering AI, collectible system, and a player stamina mechanic for exploration pacing |
| **Repository** | [github.com/Princy-2007-Cody/dusk-hollow](https://github.com/Princy-2007-Cody/dusk-hollow) |

Diagnosed and resolved a core 3D geometry compatibility bug, replacing unsupported primitives with custom-built geometry solutions to maintain rendering stability across the experience.

</details>

<details>
<summary><b>🐍 C++ Systems Projects — Snake Game & Number Guessing Engine</b></summary>
<br/>

A pair of foundational systems-level C++ projects focused on core logic, memory handling, and clean console-based UX.

| Aspect | Detail |
|---|---|
| **Stack** | C++, STL |
| **Scale** | Two standalone console applications |
| **Performance** | Real-time input handling and game-loop logic in Snake |
| **Security** | Bounds-checked input validation and game-state handling |
| **Impact** | Strengthened core algorithmic thinking and low-level control flow design |
| **Repository** | [github.com/Princy-2007-Cody/cpp-projects](https://github.com/Princy-2007-Cody/cpp-projects) |

Built to reinforce fundamentals — game loops, state machines, and input validation — that carry directly into larger systems and product engineering work.

</details>

---

### 💼 Experience

**Freelance Full Stack Developer**
*Independent / Client Projects*
`2025 — Present`

Design, build, and hand off complete web applications for clients — covering architecture, frontend/backend implementation, deployment, and post-handoff support documentation.

- Delivered end-to-end websites with backend databases and client-facing frontends
- Authored technical handoff documentation, codebase cleanup, and stack overviews for non-technical clients
- Advised on valuation and pricing for full-stack and e-commerce/marketplace properties
- Managed full project lifecycle: discovery → build → deployment → handoff

`Full Stack Development` `Client Communication` `Technical Documentation` `Deployment` `Freelance Delivery`

---

### 🏆 Achievements

<div align="center">

| Recognition | Details |
|---|---|
| 🛰️ ISRO Bharatiya Antariksh Hackathon 2026 | Built Crop-Sense AI — full ML pipeline + live dashboard for precision agriculture |
| 🚀 Shipped Product — TRIAGE | Designed and deployed a complete browser-based productivity application |
| 🎮 Independent Game Development | Built Dusk Hollow, a 3D open-world exploration game using React + Three.js |

</div>

---

### 📜 Certifications

**AWS**

![AWS Certified Cloud Practitioner](https://img.shields.io/badge/AWS%20Certified%20Cloud%20Practitioner-1E1B4B?style=for-the-badge&logo=amazonaws&logoColor=8B5CF6)

**Oracle**

![Oracle Certified Foundations Associate](https://img.shields.io/badge/Oracle%20Certified%20Foundations%20Associate-1E1B4B?style=for-the-badge&logo=oracle&logoColor=8B5CF6)

**NPTEL**

![NPTEL Certification](https://img.shields.io/badge/NPTEL%20Certified-1E1B4B?style=for-the-badge&logo=googlescholar&logoColor=8B5CF6)

**Cisco**

![Cisco Networking Basics](https://img.shields.io/badge/Cisco%20Networking%20Basics-1E1B4B?style=for-the-badge&logo=cisco&logoColor=8B5CF6)

> *Replace badge labels above with your exact certification titles and add verification links.*

---

### 💻 Coding Profiles

<div align="center">

[![LeetCode](https://img.shields.io/badge/LeetCode-1E1B4B?style=for-the-badge&logo=leetcode&logoColor=8B5CF6)](https://leetcode.com/itzz_prince_075)
[![GeeksforGeeks](https://img.shields.io/badge/GeeksforGeeks-1E1B4B?style=for-the-badge&logo=geeksforgeeks&logoColor=8B5CF6)](https://geeksforgeeks.org/user/Princy-2007-Cody)
[![HackerRank](https://img.shields.io/badge/HackerRank-1E1B4B?style=for-the-badge&logo=hackerrank&logoColor=8B5CF6)](https://hackerrank.com/Princy-2007-Cody)
[![CodeChef](https://img.shields.io/badge/CodeChef-1E1B4B?style=for-the-badge&logo=codechef&logoColor=8B5CF6)](https://codechef.com/users/Princy-2007-Cody)

</div>

---

### 📊 GitHub Analytics

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=Princy-2007-Cody&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=A78BFA&icon_color=8B5CF6&text_color=C4B5FD" width="49%" />
<img src="https://github-readme-streak-stats.herokuapp.com/?user=Princy-2007-Cody&theme=tokyonight&hide_border=true&background=0D1117&ring=8B5CF6&fire=A78BFA&currStreakLabel=C4B5FD" width="49%" />

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Princy-2007-Cody&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=A78BFA&text_color=C4B5FD" width="49%" />

</div>

---

### 🏆 GitHub Trophies

<div align="center">
<img src="https://github-profile-trophy.vercel.app/?username=Princy-2007-Cody&theme=darkhub&no-frame=true&row=1&column=6&margin-w=8" />
</div>

---

### 📈 Contribution Activity

<div align="center">
<img src="https://github-readme-activity-graph.vercel.app/graph?username=Princy-2007-Cody&theme=tokyo-night&hide_border=true&bg_color=0D1117&color=A78BFA&line=8B5CF6&point=C4B5FD" width="100%" />
</div>

---

### 🐍 Contribution Snake

<div align="center">
<img src="https://raw.githubusercontent.com/Princy-2007-Cody/Princy-2007-Cody/output/github-contribution-grid-snake-dark.svg" width="100%" />
</div>

---

### 🎯 Current Focus

```yaml
Learning:
  - Advanced ML model optimization for geospatial/time-series data
  - Distributed systems and backend scalability patterns
  - Three.js performance optimization for real-time 3D rendering

Building:
  - Crop-Sense AI: expanding multi-crop coverage and model accuracy
  - Freelance client products with production-grade handoff pipelines

Exploring:
  - Multiplayer architecture for browser-based racing games
  - Open source contribution workflows

Open To:
  - AI/ML Engineering roles
  - Full Stack freelance engagements
  - Hackathon and research collaborations
```

---

### 📫 Connect

<div align="center">

[![Gmail](https://img.shields.io/badge/Gmail-5B21B6?style=for-the-badge&logo=gmail&logoColor=white)](mailto:your.email@example.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-6D28D9?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/YOUR_LINKEDIN)
[![GitHub](https://img.shields.io/badge/GitHub-4C1D95?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Princy-2007-Cody)
[![Portfolio](https://img.shields.io/badge/Portfolio-7C3AED?style=for-the-badge&logo=vercel&logoColor=white)](https://your-portfolio-link.com)

</div>

---

<div align="center">

*"Ship the pipeline, not the prototype."*

![Footer](https://capsule-render.com/api?type=waving&color=0:24243E,50:302B63,100:0F0C29&height=150&section=footer)

</div>



<
